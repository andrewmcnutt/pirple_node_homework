...
lib.create = function(dir, file, data) {
  return new Promise(async function(resolve, reject){
    ...
    try {
      fileToCreate = await fsOpen(lib.baseDir + dir + "/" + file + ".json", "wx");
    } catch(err) {
...
lib.read = function(dir, file) {
  return new Promise(async function(resolve, reject){
    ...
    try {
      fileToRead = await fsReadFile(lib.baseDir + dir + "/" + file + ".json", "utf8");
    } catch(err) {
        reject(null);
        return;
    }
...
lib.update = function(dir, file, data) {
  return new Promise(async function(resolve, reject){
    ...
    // Open the file for writing
    try {
      fileToUpdate = await fsOpen(lib.baseDir + dir + "/" + file + ".json", "r+");
    } catch(err) {
