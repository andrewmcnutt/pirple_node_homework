...
users.post = async function(data, callback) {
  ...
  const firstName = helpers.stringLengthGreaterThanZero(data.payload.firstName);
  ...
  const streetAddress = helpers.stringLengthGreaterThanZero(data.payload.streetAddress);
  const password = helpers.stringLengthGreaterThanZero(data.payload.password);
  const allFieldsPresent = firstName && lastName && phone && email && streetAddress && password;
  ...
  // Store the user
  try {
    createNewUser = await _data.create("users", phone, userObject);
  } catch(error) {
...
users.put = async function(data, callback) {
  ...
  const streetAddress = helpers.stringLengthGreaterThanZero(data.payload.streetAddress);
  const password = helpers.stringLengthGreaterThanZero(data.payload.password);
  const updateProperties = [firstName, lastName, email, streetAddress, password];
...
users.delete = async function(data, callback) {
  ...
  try {
    ...
    callback(404, { Error : "Could not find the specified cart for deletion" });
    return;
  }
